...
class OpeningStockController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            //new validation 27/04/2023 Marco Marin
            foreach ($opening_stocks as $key => $value) {
                foreach ($value as $k => $rows) {
                    foreach ($rows as $key => $v) {
                        $stock_ini = $this->productUtil->num_uf(trim($v['quantity']));
                    }
                }
            }
            ...
            if($stock_ini > 3){
                ...
                $output = ['success' => 0,
                                 'msg' => __('lang_v1.The first stock must contain a minimum of 3 units of the product')
                            ];
            }
